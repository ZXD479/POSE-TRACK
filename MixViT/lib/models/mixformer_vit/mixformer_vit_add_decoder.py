...
from timm.models.vision_transformer import Block as Block_Decoder

from MixViT.lib.utils.misc import is_main_process
from MixViT.lib.models.mixformer.head import build_box_head, Corner_to_Box
from MixViT.lib.utils.box_ops import box_xyxy_to_cxcywh, box_cxcywh_to_xyxy

from einops import rearrange
from itertools import repeat
import collections.abc
from MixViT.lib.models.mixformer_vit.score_pred_module import ScoreDecoder
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, img_size_s=256, img_size_t=128, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, num_patches_s, decoder_embed_dim))  # learnable embeding initialized with sin-cos embedding
        self.decoder_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)
        self.decoder_blocks = nn.ModuleList([
            Block_Decoder(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
            for i in range(decoder_depth)])

        self.corner_maps_pred = nn.Linear(decoder_embed_dim, 4 ** 2 * 2, bias=True)  # decoder to patch
        self.corner_to_box = Corner_to_Box(feat_sz=img_size_s//4, stride=4)
    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        # decoder and generate corner maps
        x_s = self.decoder_embed(x_s)
        x_de_s = x_s + self.decoder_pos_embed
        for de_blk in self.decoder_blocks:
            x_de_s = de_blk(x_de_s)
        x_de_s = self.decoder_norm(x_de_s)

        # predictor projection
        x_corner = self.corner_maps_pred(x_de_s)
        x_corner = self.unpatchify(x_corner)  # N,2,H,W
        x_box = self.corner_to_box(x_corner[:, 0], x_corner[:, 1])

        return x_t_2d, x_ot_2d, x_s_2d, x_box
    ...
    def forward_test(self, x):
        ...
        x_2d = rearrange(x, 'b (h w) c -> b c h w', h=H_s, w=H_s)

        # decoder and generate corner maps
        x_s = self.decoder_embed(x)
        x_de_s = x_s + self.decoder_pos_embed
        for de_blk in self.decoder_blocks:
            x_de_s = de_blk(x_de_s)
        x_de_s = self.decoder_norm(x_de_s)
...
class MixFormerOnlineScore(nn.Module):
    ...
    def forward(self, template, online_template, search, run_score_head=True, gt_bboxes=None):
        ...
        template, online_template, search, outputs_coord = self.backbone(template, online_template, search)
        out = {'pred_boxes': outputs_coord}
        # search shape: (b, 384, 20, 20)
        # Forward the corner head and score head
        if run_score_head:
            out = self.forward_score_head(search, template, run_score_head, gt_bboxes)
    ...
    def forward_score_head(self, out_dict, search, template, run_score_head=True, gt_bboxes=None):
        """
