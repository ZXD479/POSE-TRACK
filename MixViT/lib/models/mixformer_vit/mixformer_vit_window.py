...
from MixViT.lib.models.mixformer_vit.utils import window_partition, window_unpartition
...
class Attention(nn.Module):
    ...
    def forward(self, x, num_t, num_s):
        ...
        x_t = (attn @ v_t).transpose(1, 2).reshape(1, num_t, C).repeat(B, 1, 1)
...
class Block(nn.Module):
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, window_size=0):
        super().__init__()
    ...
    def forward(self, x, num_t, num_s):
        ...
        if self.window_size > 0:
            ...
            x_s_win, pad_hw, num_windows = window_partition(x_s, self.window_size)
            x_t_win = x_t.repeat(num_windows, 1, 1)
            x = torch.cat([x_t_win, x_s_win], dim=1)
            num_s = self.window_size ** 2
        x = self.attn(x, num_t, num_s)

        # Reverse window unpartition
        if self.window_size > 0:
            x_t, x_s = torch.split(x, [num_t, num_s], dim=1)
            x_s = window_unpartition(x_s, self.window_size, pad_hw, (h_s, w_s))
            x_t = x_t[:x_s.size(0)]
            x = torch.cat([x_t, x_s], dim=1)
    ...
    def __init__(self, img_size_s=256, img_size_t=128, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                norm_layer=norm_layer, window_size=window_size) for i in range(depth)])
...
def build_mixformer_vit_window(cfg):
    backbone = get_mixformer_vit_window(cfg)  # backbone without positional encoding and attention mask
