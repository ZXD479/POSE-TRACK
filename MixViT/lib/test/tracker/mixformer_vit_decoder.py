from MixViT.lib.test.tracker.basetracker import BaseTracker
import torch
from MixViT.lib.train.data.processing_utils import sample_target
# for debug
import cv2
import os
from MixViT.lib.models.mixformer_vit import build_mixformer_vit_decoder_multi_score
from MixViT.lib.test.tracker.tracker_utils import Preprocessor_wo_mask
from MixViT.lib.utils.box_ops import clip_box
from MixViT.lib.test.tracker.tracker_utils import vis_attn_maps


class MixFormerPP(BaseTracker):
    def __init__(self, params, dataset_name):
        super(MixFormerPP, self).__init__(params)
        network = build_mixformer_vit_decoder_multi_score(params.cfg, train=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            if self.online_size <= 1:
                ...
                out_dict, _ = self.network(self.template, self.online_template, search, run_score_head=False)
                if self.params.vis_attn == 1 and self.frame_id % 200 == 0:
                    for hook in hooks:
                        hook.remove()
