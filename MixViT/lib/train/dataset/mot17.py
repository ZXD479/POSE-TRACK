...
from MixViT.lib.train.data import jpeg4py_loader
from .base_video_dataset import BaseVideoDataset
from MixViT.lib.train.admin import env_settings


class MOT17(BaseVideoDataset):
    """ MOT17 dataset.
    """
    def __init__(self, root=None, annotations=None, image_loader=jpeg4py_loader, split='train', data_fraction=None):
        """
        args:
            root        - The path to the MOT17 folder, containing the train, val, test sets.
        ...
        """
        self.root = env_settings().mot17_dir if root is None else root
        self.annotations=os.path.join(root,'annotations') if annotations is None else annotations
        super().__init__('MOT17', root, image_loader)

        self.split=split
        self.sequence_list=[f[:-4] for f in os.listdir(os.path.join(annotations,self.split))]
    ...
    def _get_frame(self, seq_id, frame_id):
        ...
        img1_path=os.path.join(self.root,'train',seq_name,'img1')
        if self.split=='val_half':
            num_images=len([i for i in os.listdir(img1_path) if 'jpg' in i])
            frame_id=frame_id+num_images // 2 + 1
        frame_path = os.path.join(self.root,'train',seq_name,'img1','{:0>6d}.jpg'.format(frame_id))
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        object_meta = OrderedDict({'object_class_name': 'player',
                                   'seq':self.sequence_list[seq_id][:-4],
