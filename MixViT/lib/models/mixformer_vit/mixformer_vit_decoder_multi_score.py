...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, img_size_s=256, img_size_t=128, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.de_pos_embed_s = nn.Parameter(torch.zeros(1, self.num_patches_s, decoder_embed_dim), requires_grad=False)
        self.de_pos_embed_t = nn.Parameter(torch.zeros(1, self.num_patches_t, decoder_embed_dim), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        de_pos_embed_t = get_2d_sincos_pos_embed(self.de_pos_embed_t.shape[-1], int(self.num_patches_t ** .5),
                                                 cls_token=False)
        self.de_pos_embed_t.data.copy_(torch.from_numpy(de_pos_embed_t).float().unsqueeze(0))

        de_pos_embed_s = get_2d_sincos_pos_embed(self.de_pos_embed_s.shape[-1], int(self.num_patches_s ** .5),
                                                 cls_token=False)
        self.de_pos_embed_s.data.copy_(torch.from_numpy(de_pos_embed_s).float().unsqueeze(0))
    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        x_s = x_s + self.de_pos_embed_s
        x_t = x_t + self.de_pos_embed_t
        x_ot = x_ot + self.de_pos_embed_t
        x = torch.cat([x_t, x_ot, x_s], dim=1)
        ...
        x_t_2d = x_t.transpose(1, 2).reshape(B, -1, H_t, W_t)
        x_ot_2d = x_ot.transpose(1, 2).reshape(B, -1, H_t, W_t)
        x_s_2d = x_s.transpose(1, 2).reshape(B, -1, H_s, W_s)
...
def get_mixformer_vit(config, train):
    ...
    if config.MODEL.VIT_TYPE == 'large_patch16':
        vit = VisionTransformer(
            img_size_s=img_size_s, img_size_t=img_size_t, decoder_depth=4,
            patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_path_rate=0.1)
    elif config.MODEL.VIT_TYPE == 'base_patch16':
        vit = VisionTransformer(
            img_size_s=img_size_s, img_size_t=img_size_t, decoder_depth=8,
...
def build_mixformer_vit_decoder_multi_score(cfg, settings=None, train=True):
