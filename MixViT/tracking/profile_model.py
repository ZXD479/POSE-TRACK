...
from MixViT.lib.models.mixformer.mixformer_online import Attention
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='mixformer_online', choices=['mixformerpp_vit_multi', 'mixformer_vit', 'mixformer_online', 'mixformer_vit_window'],
...
def evaluate(model, template, search, skip=200, display_info='MixFormer'):
    ...
    with torch.no_grad():
        ...
        start = time.time()
        for i in range(T_t):
            model(template, template, search, run_score_head=False)
        end = time.time()
...
if __name__ == "__main__":
    ...
    if args.script == "mixformer_vit":
        model_constructor = model_module.build_mixformer_sparse_vit
    ...
    elif args.script == "mixformerpp_vit_multi":
        model_constructor = model_module.build_mixformerpp_vit_multi_score
        model = model_constructor(cfg)
        # get the template and search
        template = get_data(bs, z_sz)
        search = get_data(bs, x_sz)
