...
class DanceTrack(BaseVideoDataset):
    """ DanceTrack dataset.
    """
    def __init__(self, root=None, annotations=None, image_loader=jpeg4py_loader, split='train', data_fraction=None):
        """
        args:
            root        - The path to the DanceTrack folder, containing the train, val, test sets.
        ...
        """
        self.root = env_settings().dancetrack_dir if root is None else root
        self.annotations=os.path.join(root,'annotations') if annotations is None else annotations
        super().__init__('DanceTrack', root, image_loader)
    ...
    def _get_frame(self, seq_id, frame_id):
        ...
        frame_path = os.path.join(self.root,self.split,seq_name,'img1','{:0>8d}.jpg'.format(frame_id))
