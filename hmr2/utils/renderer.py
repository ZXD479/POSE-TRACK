...
def cam_crop_to_full(cam_bbox, box_center, box_size, img_size, focal_length=5000.):
    ...
    cx = torch.tensor([array[0] for array in box_center]).to('cuda')
    cy = torch.tensor([array[1] for array in box_center]).to('cuda')
    b = box_size.to('cuda')
...
class Renderer:
    ...
    def vertices_to_trimesh(self, vertices, camera_translation, track_id, mesh_base_color=(1.0, 1.0, 0.9),  
        ...
        #     baseColorFactor=(*mesh_base_color, 1.0))
        #颜色
# 在 Renderer 类的 vertices_to_trimesh 方法中修改颜色调色板
        COLOR_PALETTE = [
            (0.85, 0.15, 0.13),  # 1. 胭脂红
            (0.96, 0.28, 0.12),  # 2. 朱红色
            (1.00, 0.50, 0.00),  # 3. 橙黄
            (0.98, 0.83, 0.14),  # 4. 琥珀黄
            (0.80, 0.95, 0.15),  # 5. 柠檬绿
            (0.22, 0.85, 0.30),  # 6. 翡翠绿
            (0.18, 0.80, 0.72),  # 7. 孔雀蓝
            (0.10, 0.45, 0.90),  # 8. 钴蓝色
            (0.35, 0.30, 0.95),  # 9. 蓝紫色
            (0.62, 0.25, 0.95),  # 10. 紫罗兰
            (0.89, 0.15, 0.65),  # 11. 品红色
            (0.95, 0.15, 0.35),  # 12. 珊瑚红
            (0.50, 0.10, 0.85),  # 13. 深紫色
            (0.15, 0.65, 0.45),  # 14. 薄荷绿
            (0.95, 0.90, 0.25),  # 15. 金丝雀黄
            (0.25, 0.95, 0.80),  # 16. 碧绿色
            (0.05, 0.75, 0.95),  # 17. 天蓝色
            (0.95, 0.65, 0.05),  # 18. 琥珀橙
            (0.10, 0.30, 0.95),  # 19. 宝蓝色
            (0.95, 0.10, 0.10)   # 20. 正红色
        ]
        vertex_colors = np.array([(*COLOR_PALETTE[track_id], 1.0)] * vertices.shape[0])
    ...
    def render_rgba_multiple(
        ...
        mesh_list = [pyrender.Mesh.from_trimesh(self.vertices_to_trimesh(vvv, ttt.copy(), iii, mesh_base_color, rot_axis, rot_angle)) for vvv,ttt,iii in zip(vertices, cam_t, track_id)]
